...
doviz = True # visualize the plots or not
dosave = False # save files or not
...
model = Sequential()
if dropout == False: # TODO : Change to "False" to use dropout
...
batch_size       =  32  # size of the minibatch (each batch will contain 32 sentences)
...
if doviz:
    ...
    if dosave:
        if dropout:
            plt.savefig('accuracy_w_dropout.jpg')
        else:
            plt.savefig('accuracy_w_o_dropout.jpg')

# Remember the scores
test_loss_w_o_dropout = 0.616227714682
test_acc_w_o_dropout = 0.822866666667
test_loss_w_dropout = 0.367138043578
test_acc_w_dropout = 0.839466666667
